...
ARG LW_SCANNER_VERSION=0.2.1
RUN apk add --no-cache curl
RUN curl -LJo /usr/local/bin/lw-scanner https://github.com/lacework/lacework-vulnerability-scanner/releases/download/v0.2.1/lw-scanner-linux-$(if [ $(uname -m) == "x86_64" ]; then echo "amd64"; elif [ $(uname -m) == "aarch64" ]; then echo "arm64"; fi) && chmod +x /usr/local/bin/lw-scanner
